...
/**
  ...
  @return Return 0 or 1 if an error occurred.
 */
static int init_server_auto_options( bool generate_uuid = true )
{
  if ( strlen(server_uuid) > 0 ) 
  {
	  sql_print_information("server_uuid exists [%s]", server_uuid );
	  return 0;
  }
  bool flush= false;
...
#endif
{
  ...
  */
  ...
  if (!opt_bootstrap)
  {
	  //move it before init_server_components
