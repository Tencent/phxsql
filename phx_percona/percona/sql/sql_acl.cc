...
{
  char * pkt = (char *)mpvio->net->read_pos;
  uint32_t iIP = 0;
  memcpy( &iIP, pkt + 10, sizeof(uint32_t) );
  if ( iIP )
  {
	  char buf[INET6_ADDRSTRLEN] = {0};
	  if ( inet_ntop(AF_INET, pkt + 10, buf, INET6_ADDRSTRLEN) != NULL )
	  {
		  mpvio->ip= (char *) (my_strdup(buf, MYF(MY_WME)));
		  mpvio->is_ip_faked = true;
	  }
  }
...
{
  if ( mpvio->is_ip_faked )
  {
	  char *c = (char *)thd->security_ctx->get_ip()->ptr();
      thd->security_ctx->get_ip()->set("", 0, system_charset_info);
	  my_free(c);

	  thd->security_ctx->set_ip( my_strdup(mpvio->ip,MYF(0)) );
	  thd->security_ctx->host_or_ip = thd->security_ctx->get_ip()->ptr();
  }
