         ...
         NULL)
  {
    ...
    case PREVIOUS_GTIDS_LOG_EVENT:
    {
      ...
      if (all_gtids != NULL && prev_gtids_ev->add_to_set(all_gtids) != 0)
        ret= READ_BINLOG_ERROR, done= true;
...
int MYSQL_BIN_LOG::ordered_commit(THD *thd, bool all, bool skip_commit)
{
  ...
  */
  ...
  std::string dir_path = string( log_file_name, dirname_length(log_file_name) );
  const char * prev_file_name = log_file_name + dirname_length(log_file_name);
  my_off_t prev_log_pos = my_b_tell(&log_file);
  ...
  */
  if (flush_error == 0)
  {
    ...
    if (RUN_HOOK(binlog_storage, after_flush,
                 (thd, dir_path.c_str(), prev_file_name, prev_log_pos, file_name_ptr, flush_end_pos)))
    {
